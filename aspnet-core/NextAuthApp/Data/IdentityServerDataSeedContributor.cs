...
public class IdentityServerDataSeedContributor : IDataSeedContributor, ITransientDependency
{
    ...
    private async Task CreateClientsAsync()
    {
        ...
        // Angular Client
        var consoleAndAngularClientId = configurationSection["NextAuthApp_App:ClientId"];
        if (!consoleAndAngularClientId.IsNullOrWhiteSpace())
        {
            var webClientRootUrl = configurationSection["NextAuthApp_App:RootUrl"]?.TrimEnd('/');

            await CreateClientAsync(
                name: consoleAndAngularClientId,
                scopes: commonScopes,
                grantTypes: new[] { "password", "client_credentials", "authorization_code" },
                secret: (configurationSection["NextAuthApp_App:ClientSecret"] ?? "1q2w3e*").Sha256(),
        ...
        {
            var swaggerRootUrl = configurationSection["NextAuthApp_Swagger:RootUrl"].TrimEnd('/');

            await CreateClientAsync(
                name: swaggerClientId,
                scopes: commonScopes,
                grantTypes: new[] { "authorization_code" },
                secret: configurationSection["NextAuthApp_Swagger:ClientSecret"]?.Sha256(),
                requireClientSecret: false,
